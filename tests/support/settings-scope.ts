/**
 * Local test double for the harness settings scope (mirrors the published
 * `dsh-client-test-runtime`'s `stubSettingsScope`). The published package
 * can't load standalone because it imports unshipped ui-renderer source paths.
 */
import { vi } from 'vitest'
import type { SettingsScope, SettingsScopeSnapshot } from './runtime-double.ts'

/** Handle over one stubbed scope: the scope face, its write spy, and publication controls. */
